        ...
        <div class="pd-ltr-20 xs-pd-20-10">
            <div class="min-height-200px">
                            ...
                            <table class="table hover multiple-select-row data-table-export nowrap" id="datatableid">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{trans('web.UserId')}}</th>
                                        <th>{{trans('web.User Name')}}</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$message->user->agent == 1 ? 'agent' : 'user'}}</td>
                ...
                <div class="mb-30">
                    <div class="card-box p-3">
                        <div class="title mb-3">
                            <h4>{{ trans('web.Recieve Messages') }}</h4>
                        </div>
                        <div class="table-responsive">
                            <table class="table hover multiple-select-row data-table-export nowrap" id="datatableid_2">
                                ...
                                <tbody>
                                    @if ($messages)
                                    @foreach ($RecieveMessages as $message)
                                    <tr>
                                        <td class="table-plus">{{$message->id}}</td>
                                        @if ($message->user_id == null)
                                            <td>{{$message->user_name}}</td>
                                            <td>{{$message->email}}</td>
                                            <td>{{ trans('web.not registered') }}</td>
                                        @else
                                            <td>{{$message->user->name}}</td>
                                            <td>{{$message->user->email}}</td>
                                            <td>{{$message->user->type == 1 ? 'agent' : 'user'}}</td>
                                        @endif
        ...
        <div class="modal fade customscroll" id="message-add" tabindex="-1" role="dialog">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{aurl('messages/add')}}" method="POST">
                        @csrf
                        <div class="modal-body p-3">
                            <div class="form-group row">
                                <label class="col-md-4">{{ trans('web.User ID') }}</label>
                                <div class="col-md-8">
